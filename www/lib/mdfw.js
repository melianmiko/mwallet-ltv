 ...
 */class FWSettingsScreen extends Screen{/**
     ...
     */static get LOCALE(){return{cancel:"Cancel",ok:"Ok",apply:"Restart and apply",darkTheme:"Dark theme",bigMode:"Interface scale",restartRequired:"Application restart required to apply this changes.",titleColorAccent:"Color",customColor:"Set custom color"}}/**
     * Colors palette
     */static get COLORS(){return["#f44336","#E91E63","#9C27B0","#9C27B0","#3F51B5","#2196F3","#03A9F4","#00BCD4","#009688","#4CAF50","#8BC34A","#CDDC39","#FFEB3B","#FFC107","#FF9800","#FF5722","#795548","#9E9E9E","#607D8B"]}/**
     * OnCreate event override
     * @param {Object} loc Localization override
     */onCreate(loc){var locale=FWSettingsScreen.LOCALE,ctx=this;if(loc)for(var a in loc)locale[a]=loc[a];this.addMod(new RightSideScreenMod);this.setHomeAsUpAction();this.addAction(new MenuItem(locale.apply,"check",()=>{location.reload()}));this.locale=locale;this.appendView(new Checkbox().setTitle(locale.darkTheme).setChecked(localStorage.fw_cfg_nightmode==="true").setOnCheckedListener(isChecked=>{localStorage.fw_cfg_nightmode=isChecked?"true":""}));this.appendView(new Checkbox().setTitle(locale.bigMode).setChecked(localStorage.fw_cfg_font_size==="1.25").setOnCheckedListener(isChecked=>{if(isChecked)localStorage.fw_cfg_font_size="1.25";else localStorage.fw_cfg_font_size=""}));this.appendView(new SubHeader(locale.titleColorAccent));this._palette=Utils.inflate({type:"div",class:"fw-palette"});this.appendView(this._palette);this.appendView(new RowView().setTitle(locale.customColor).setIcon("palette").setOnClickListener(function(){ctx.customColorDialog()}));this.updatePalette()}customColorDialog(){var ctx=this;var te=new TextInput().setTitle(this.locale.customColor).setType("color").fromString("#000000");var d=new Dialog().addButton(new Button().setText(this.locale.ok).setOnClickListener(function(){localStorage.fw_main_color=te.toString();ctx.updatePalette();d.hide()})).addButton(new Button().setText(this.locale.cancel).setOnClickListener(function(){d.hide()})).appendView(te).show()}updatePalette(){this._palette.innerHTML="";this.addColor(Config.defaultColor);for(var a in FWSettingsScreen.COLORS){this.addColor(FWSettingsScreen.COLORS[a])}}addColor(color){var ctx=this;var view=Utils.inflate({type:"div",class:"color "+(Config.mainColor==color?"selected":"")});view.onclick=function(){localStorage.fw_main_color=color;ctx.updatePalette()};view.style.backgroundColor=color;this._palette.appendView(view)}/**
